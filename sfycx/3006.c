...
int main(){
    ...
    scanf("%d",&n);
    char name[100][21];
    int final_score[100],class_score[100],is_leader[100],is_west[100],paper_count[100];
    for(int i=0;i<n;i++){
        scanf("%s %d %d %c %c %d",name[i],&final_score[i],&class_score[i],&is_leader[i],&is_west[i],&paper_count[i]);
    }
    int max_bonus=0,total_bonus=0,max_index=0;
    for(int i=0;i<n;i++){
        int bonus=0;
        if(final_score[i]>80&&paper_count[i]>=1){
            bonus+=8000;
        }
        if(final_score[i]>85&&class_score[i]>80){
            bonus+=4000;
        }
        if(final_score[i]>90){
            bonus+=2000;
        }
        if(final_score[i]>85&&is_west[i]=='Y'){
            bonus+=1000;
        }
        if(class_score[i]>80&&is_leader[i]=='Y'){
            bonus+=850;
        }
        if(bonus>max_bonus){
            max_bonus=bonus;
