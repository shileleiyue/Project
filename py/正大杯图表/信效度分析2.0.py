# -*- coding: utf-8 -*-
"""
影视行业调研分析脚本（使用新汇总表绘图，修正选项匹配问题）
功能：
1. 信效度分析（针对Q6-Q10、Q18，计算总α及删除项后的α）
2. 单变量图表：从新汇总表 "350881459_1772196647a7Xnny (1).xlsx" 读取数据并绘制
3. 双变量交叉分析、相关性分析、建议词云（仍使用原始数据 "350881459_按序号_关于影视行业环境与观众沉浸体验研究的详细问卷_274_274 (1).xlsx"）
"""
...
# ==================== 配置 ====================
RAW_DATA_FILE = "350881459_按序号_关于影视行业环境与观众沉浸体验研究的详细问卷_274_274 (1).xlsx"
SUMMARY_TABLE_FILE = "350881459_1772196647a7Xnny (1).xlsx"  # 新上传的汇总表
OUTPUT_DIR = "分析结果"
...
# ==================== 信度分析 ====================
def reliability_analysis_immersion(df):
    print("\n" + "=" * 50)
    print("开始信度分析（沉浸度低为正方向）")
    print("=" * 50)
    ...
    data = pd.DataFrame()
    data['Q6'] = df[q_cols[6]].map({1: 5, 2: 4, 3: 3, 4: 2, 5: 1})  # 反向
    data['Q7'] = df[q_cols[7]].copy()  # 正向
    data['Q8'] = df[q_cols[8]].copy()  # 正向
    data['Q9'] = df[q_cols[9]].copy()  # 正向
    data['Q10'] = df[q_cols[10]].map({1: 4, 2: 3, 3: 2, 4: 1})  # 反向
    data['Q18'] = df[q_cols[18]].map({1: 5, 2: 4, 3: 3, 4: 2, 5: 1})  # 反向
    ...
    alpha_total = pg.cronbach_alpha(data)[0]
    print(f"\n总体 Cronbach's α = {alpha_total:.3f}")
    if alpha_total >= 0.9:
        print("信度非常好")
    ...
    desc = data.describe().T[['mean', 'std']].round(3)
    desc.columns = ['均值', '标准差']
    print("\n各题目描述性统计：")
    print(desc)
    ...
    df_item = pd.DataFrame(item_stats)
    print("\n删除每个题目后的Cronbach's α：")
    print(df_item)

    with pd.ExcelWriter(os.path.join(OUTPUT_DIR, "信度分析结果（沉浸度）.xlsx")) as writer:
        pd.DataFrame({'总信度': [alpha_total]}).to_excel(writer, sheet_name='总信度', index=False)
        desc.to_excel(writer, sheet_name='描述性统计')
        df_item.to_excel(writer, sheet_name='删除项后信度', index=False)
    print(f"\n结果已保存至: {os.path.join(OUTPUT_DIR, '信度分析结果（沉浸度）.xlsx')}")
...
def validity_analysis_immersion(df):
    ...
    try:
        ...
        if 'force_all_finite' in str(e):
            print("\n因子分析出错：可能是因为 scikit-learn 版本不兼容。")
            print("请尝试安装兼容版本：pip install scikit-learn==1.1.3")
            return
        else:
            raise e
    ...
    out_path = os.path.join(OUTPUT_DIR, "效度分析结果（沉浸度）.xlsx")
    ...
    for i, row in enumerate(rows):
        ...
        if first_cell and re.match(r'^\d+\.', first_cell) and ('单选题' in first_cell or '多选题' in first_cell):
            ...
            if current_q is not None and current_options and current_counts:
                q_data[current_q] = (current_options, current_counts)
            # 提取题号
...
def main():
    print("=" * 50)
    print("影视行业调研分析脚本（使用新汇总表绘图，修正选项匹配）")
    print("=" * 50)
    ...
    print("第一部分：信效度分析")
    print("=" * 50)
    reliability_analysis_immersion(df_raw)
